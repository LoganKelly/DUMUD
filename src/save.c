...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->act				= PLR_NOSUMMON + PLR_AUTODAMAGE + PLR_AUTOLOOT + PLR_AUTOSAC + PLR_AUTOGOLD + PLR_AUTOEXIT;
    ch->comm				= COMM_COMBINE 
					| COMM_PROMPT;
    ch->prompt 				= str_dup( "{W-={WHP:{x{R%h/%H {WMP:{x{B%m/%M {WMV:{x{C%v/%V {WTNL:{x{M%X%b{W=-{x " );
