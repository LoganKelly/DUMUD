...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
	...
	return;
	 ...
	 if ( dam ==   0 ) { vs = "miss";	vp = "misses";		}
    ...
    else if ( dam <=  20 ) { vs = "{mwound{x";	vp = "{mwounds{x";		}
    else if ( dam <=  24 ) { vs = "{mmaul{x";       vp = "{mmauls{x";		}
    ...
    else if ( dam <=  52 ) { vs = "{RM{YA{RS{YS{RA{YC{RR{YE{x";	vp = "{RM{YA{RS{YS{RA{YC{RR{YE{RS{x";	}
    else if ( dam <=  56 ) { vs = "{RM{MA{RN{MG{RL{ME{x";	vp = "{RM{MA{RN{MG{RL{ME{RS{x";		}
    else if ( dam <=  60 ) { vs = "{Y*** {MDEMOLISH {Y***{x";
			     vp = "{Y*** {MDEMOLISHES {Y***{x";			}
    else if ( dam <=  75 ) { vs = "{Y*** {RDEVASTATE {Y***{x";
			     vp = "{Y*** {RDEVASTATES {Y***{x";			}
			     ...
			     vp = "{rdoes {GU{bN{MS{WP{RE{rA{CK{YA{BB{gL{mE {rthings to{x";		}
    punct   = (dam <= 24) ? '.' : '!';
	...
	else
	{
		...
		if (IS_SET(ch->act,PLR_AUTODAMAGE))
	    	...
	    	sprintf( buf1, "$n's %s %s $N%c",  attack, vp, punct );
            	sprintf( buf2, "Your %s %s $N%c {G[%d]{x",  attack, vp, punct, dam );
               }
		else if (IS_SET(victim->act,PLR_AUTODAMAGE))
		{
		sprintf( buf1, "$n's %s %s $N%c",  attack, vp, punct );
		sprintf( buf3, "$n's %s %s you%c {R[%d]{x", attack, vp, punct, dam );
		}
               else
                {		
		sprintf( buf1, "$n's %s %s $N%c",  attack, vp, punct );
