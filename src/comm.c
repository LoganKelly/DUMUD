 ...
 */
        ...
        if ((victim = ch->fighting) != NULL)
        {
            if (victim->max_hit > 0)
                ...
                percent = -1;
            if (percent >= 100)
                sprintf(wound,"{WEnemy: [{R+++{Y+++{G++++{W]{x");
            else if (percent >= 90)
                sprintf(wound,"{WEnemy: [{R+++{Y+++{G+++ {W]{x");
            else if (percent >= 80)
                sprintf(wound,"{WEnemy: [{R+++{Y+++{G++  {W]{x");
                ...
                sprintf(wound,"{WEnemy: [{R+++{Y+++    {W]{x");
            else if (percent >= 45)
                sprintf(wound,"{WEnemy: [{R+++{Y++     {W]{x");
            else if (percent >= 30)
                sprintf(wound,"{WEnemy: [{R+++{Y+      {W]{x");
            else if (percent >= 28)
                sprintf(wound,"{WEnemy: [{R+++{W       {W]{x");
            else if (percent >= 15)
                sprintf(wound,"{WEnemy: [{R++{W        {W]{x");
            else if (percent >= 8)
                sprintf(wound,"{WEnemy: [{R+{W         {W]{x");
            else
                sprintf(wound,"{WEnemy: [          {W]{x");
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
    ...
    case CON_ANSI:
        ...
        {
            d->ansi = TRUE;
            write_to_desc_color(d, "\n\r{R{YA{Bn{Cs{Gi {Me{Wn{Ya{Bb{Rl{Ce{G{Yd{M!{x\n\r" );
            d->connected = CON_GET_NAME;
            {
                extern char * help_greeting;
                if ( help_greeting[0] == '.' )
                  write_to_desc_color(d, help_greeting+1 );
                else
                  write_to_desc_color(d, help_greeting  );
            }
            break;
        }
        ...
        if (UPPER(argument[0]) == 'N')
        {
            ...
            d->connected = CON_GET_NAME;
            {
                extern char * help_greeting_nocolor;
                if ( help_greeting_nocolor[0] == '.' )
                  write_to_buffer( d, help_greeting_nocolor+1, 0 );
                else
                  write_to_buffer( d, help_greeting_nocolor, 0  );
            }
            break;
        }
        else
...
void write_to_desc_color (DESCRIPTOR_DATA * d, const char *txt)
{
  const char *point;
  char *point2;
  char buf[MAX_STRING_LENGTH * 4];
