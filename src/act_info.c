...
/* REVIEW (keithkel): this code is pathetically bad.  Rewrite it.  Entirely. */
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf( buf, "{B||{x{WTrust level:{x %d.",
	    get_trust( ch ) );
	send_to_char( buf, ch );
        length = 70 - strlen_color(buf);
        for ( iLength=0; iLength < length; iLength ++ )
    ...
    sprintf(buf, "{B||{x{WRace:{x %s  {WSex:{x %s {WClass:{x %s",
    ...
    sprintf( buf,
	"{B||{x{WPractices:{x %d {WTrains:{x %d",
    ...
    sprintf( buf,
	"{B||{x{W# of Items:{x %d/%d (%ld/%d pounds)",
    ...
    sprintf( buf,
	"{B||{x{WStr:{x {c%d{C(%d)  {WInt:{x {c%d{C(%d)  {WWis:{x {c%d{C(%d)  {WDex:{x {c%d{C(%d)  {WCon:{x {c%d{C(%d){x",
    ...
    sprintf( buf,
	"{B||{x{WXP:{x %d {WMoney:{x %ld gold , %ld silver",
    ...
    sprintf( buf, "{B||{x{WWimpy:{x %d", ch->wimpy );
    send_to_char( buf, ch );
    length = 70 - strlen_color(buf);
    for ( iLength=0; iLength < length; iLength ++ )
	{
	...
	{
	send_to_char( "{B||{xYou are drunk.  ",   ch );
	for ( iLength=0; iLength < 52; iLength ++ )
	{
	sprintf(buf, " ");
	send_to_char( buf, ch );
	}
	...
	{
	send_to_char( "{B||{xYou are thirsty.", ch );
	for ( iLength=0; iLength < 52; iLength ++ )
	{
	sprintf(buf, " ");
	send_to_char( buf, ch );
	...
	{
	send_to_char( "{B||{xYou are hungry. ",  ch );
	for ( iLength=0; iLength < 52; iLength ++ )
	{
	sprintf(buf, " ");
	send_to_char( buf, ch );
    ...
    {
    case POS_DEAD:     
	send_to_char( "{B||{xYou are DEAD!!           ",		ch );
	break;
    case POS_MORTAL:
	send_to_char( "{B||{xYou are mortally wounded.",	ch );
	break;
    case POS_INCAP:
	send_to_char( "{B||{xYou are incapacitated.   ",	ch );
	break;
    case POS_STUNNED:
	send_to_char( "{B||{xYou are stunned.         ",		ch );
	break;
    case POS_SLEEPING:
	send_to_char( "{B||{xYou are sleeping.        ",		ch );
	break;
    case POS_RESTING:
	send_to_char( "{B||{xYou are resting.         ",		ch );
	break;
    case POS_SITTING:
	send_to_char( "{B||{xYou are sitting.         ",		ch );
	break;
    case POS_STANDING:
	send_to_char( "{B||{xYou are standing.        ",		ch );
	break;
    case POS_FIGHTING:
	send_to_char( "{B||{xYou are fighting.        ",		ch );
	break;
    }
    for ( iLength=0; iLength < 43; iLength ++ )
	{
	sprintf(buf, " ");
	send_to_char( buf, ch );
	}
    ...
    /* print AC values */
	sprintf( buf,"{B||{x{WArmor: {ypierce: {Y%d  {ybash: {Y%d  {yslash: {Y%d  {ymagic: {Y%d{x",
		 GET_AC(ch,AC_PIERCE),
		 GET_AC(ch,AC_BASH),
		 GET_AC(ch,AC_SLASH),
		 GET_AC(ch,AC_EXOTIC));
	...
	send_to_char(buf,ch);
    }                    Kills stupid AC names*/


    /* RT wizinvis and holy light */
    if ( IS_IMMORTAL(ch))
    {
      send_to_char("{B||{x{WHoly Light:{x ",ch);
      ...
      if (ch->invis_level)
      {
        sprintf( buf, "  {WInvisible:{x level %d",ch->invis_level);
        send_to_char(buf,ch);
      }
      else
      {
      ...
      if (ch->incog_level)
      {
	sprintf(buf,"  {WIncognito:{x level %d",ch->incog_level);
	send_to_char(buf,ch);
      }
      else
      {
    ...
    if ( ch->level >= 15 )
    {
	sprintf( buf, "{B||{x{WHitroll:{x %d  {WDamroll:{x %d.",
...
void do_whois (CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(arg,wch->name))
	{
		...
		wch->pcdata->cname != NULL ? wch->pcdata->cname : wch->name, IS_NPC(wch) ? "" : wch->pcdata->title);
	    add_buf(output,buf);
	}
    }
