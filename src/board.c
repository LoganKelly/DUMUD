...
/* Return the number of total notes 'ch' has in 'board' */
/* Returns BOARD_NOACCESS if ch has no access to board */
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
		send_to_char (BOLD RED "Num         Name Unread Total Description" NO_COLOR "\n\r"
		              RED "=== ============ ====== ===== ===========" NO_COLOR "\n\r",ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
			...
			{ 
				sprintf (buf, BOLD "%2d" NO_COLOR "> " GREEN BOLD "%12s" NO_COLOR " [%s%4d" NO_COLOR "] " NO_COLOR "[%s%3d" NO_COLOR "] "YELLOW "%s" NO_COLOR "\n\r", 
				                   count, boards[i].short_name, unread ? RED : GREEN,  
				                    unread, total ? YELLOW_BG : GREEN, total, boards[i].long_name);
				send_to_char (buf,ch);	                    
				count++;
			} /* if has access */
